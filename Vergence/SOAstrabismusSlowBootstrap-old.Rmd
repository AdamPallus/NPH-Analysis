...
# saveRDS(xx,'SOAstrab_slow_bootstrap.RDS')
...
signifall<-readRDS('SOAbootstrapCInormalStrab.RDS')
...
signifall$exo<- as.factor(signifall$exo)
levels(signifall$exo) <- c('Eso','Exo')
...
wilcox.test(abs(median.velocity) ~ exo,data=filter(signifall,strabismic))

wilcox.test(abs(median.velocity) ~ strabismic,data=signifall)
```
