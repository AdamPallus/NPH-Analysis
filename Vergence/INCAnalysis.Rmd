...
if (LoadFromFile){
  ...
  o<- readRDS('INCsaccadesmarked-DC.RDS')
}else{
  o<-NULL
}
...
# t<- loadnewcsv2(path="C:/Users/setup/Desktop/NRTP Vergence/INCstrab/DCcoils/",referencefile=o)
t<- loadnewcsv2(path="C:/Users/setup/Desktop/NRTP Vergence/INCstrab/new/",referencefile=o)
...
t %>%
  # mutate(smi= monkey=='DC' & cellnum<=921)%>%
  group_by(neuron) %>%
  do(measureSaccadesINC(.))-># %>%
  # do(expandSaccades(.,buffer=80))->
  t

# saveRDS(t,'INCsaccadesmarked-DC.RDS')
...
compare2eyes<- function(zp,zm,c1,maxEP=25,color1='black',color2='black',
                         c1name=NA){
...
RV<-ggplot(zp %>% filter(neuron==c1))+
  geom_point(aes(mean.R.V,meanFR),color=color1)+
  stat_smooth(aes(mean.R.V,meanFR),color='black',se=FALSE,method=lm)+
  xlab('')+
  theme_minimal()+
  ylab(paste(c1name,'\nMean Firing Rate (spk/s)'))

RH<-ggplot(zp %>% filter(neuron==c1))+
  geom_point(aes(mean.R.H,meanFR),color=color1)+
  stat_smooth(aes(mean.R.H,meanFR),color='black',se=FALSE,method=lm)+
  xlab('')+
  ylab('')+
  theme_minimal()

  LV<-ggplot(zp %>% filter(neuron==c1))+
    geom_point(aes(mean.L.V,meanFR),color=color2)+
    stat_smooth(aes(mean.L.V,meanFR),color='black',se=FALSE,method=lm)+
    xlab('Mean Vertical Eye Position (deg)')+
    ylab(paste(c1name,'\nMean Firing Rate (spk/s)'))+
    theme_minimal()
  
  LH<-ggplot(zp %>% filter(neuron==c1))+
    geom_point(aes(mean.L.H,meanFR),color=color2)+
    stat_smooth(aes(mean.L.H,meanFR),color='black',se=FALSE,method=lm)+
    xlab('Mean Horizontal Eye Position (deg)')+
    ylab('')+
    theme_minimal()
...
dir.imp<-ggplot(zm %>% filter(neuron==c1))+
  # geom_segment(aes(y=0,yend=1,x=dir.pref.imp,xend=dir.pref.imp),color='black')+
  geom_segment(aes(y=0,yend=1,x=dir.imp.R,xend=dir.imp.R),color='black')+
   geom_segment(aes(y=0,yend=R2R,x=dir.imp.R,xend=dir.imp.R,color=neuron),size=2)+
  # scale_color_manual(values=color.values)+
  scale_x_continuous(limits=c(-180,180),breaks=c(0,90,180,-90))+
  coord_polar(start=pi/2,direction=-1)+
  theme_minimal()+
...
dir.slope<-ggplot(zm %>% filter(neuron == c1))+
  geom_segment(aes(y=0,yend=1,x=dir.imp.L,xend=dir.imp.L),color='black')+
  geom_segment(aes(y=0,yend=R2L,x=dir.imp.L,xend=dir.imp.L,color=neuron),size=2)+
  # scale_color_manual(values=color.values)+
  scale_x_continuous(limits=c(-180,180),breaks=c(0,90,180,-90))+
  coord_polar(start=pi/2,direction=-1)+
  theme_minimal()+
...
return(list(RV,LV,dir.slope,RH,LH,dir.imp))
...
zp %>%
  filter(meanFR>FRthresh,abs(mean.V)<5) %>%
  separate(neuron,c('monkey','cellnum'),remove=FALSE) %>%
  group_by(neuron,monkey) %>%
  summarize(mean.CV=mean(sd.sdf/meanFR,na.rm=T),
            mean.FR=mean(meanFR,na.rm=T))->
...
zp %>%
  filter(meanFR>FRthresh,mean.V > 8, mean.V< 12) %>%
  separate(neuron,c('monkey','cellnum'),remove=FALSE) %>%
  group_by(neuron,monkey) %>%
...
zp %>%
  filter(meanFR>FRthresh,mean.V> -12,mean.V< -8) %>%
  separate(neuron,c('monkey','cellnum'),remove=FALSE) %>%
  group_by(neuron,monkey) %>%
...
```{r}
cvmeans2<-left_join(cvmeans,select(zmc,dir.pref.imp),by='neuron')
cvmeans2<- mutate(cvmeans2,skewness=abs(90-abs(dir.pref.imp)))

ggplot(cvmeans2)+
  geom_point(aes(mean.CVmin,mean.FRmax,color=monkey,size=skewness))+
  ggtitle('min')+
  scale_x_log10()+geom_vline(xintercept = 0.5)+theme(legend.position='bottom')
```
