#I want a function in one place where I can do all the various things I do with saccades
#1) mark saccades
#2) number saccades
#3) number saccades and fixations
#4) reject saccades where the peak velocity is too high or the duration is too low
#5) add a buffer around saccades, but handle overlapping buffers intelligently





addBuffer<- function(stimes,bufferlength){
...
joinsaccades<-function(tt,buffer=20,threshold=30){
  ...
  stimes<-findSaccades(tt$conj.velocity,threshold)
  stimes %>%
    mutate(s=row_number(),
           saccade.onset=saccade.onset-buffer,
           saccade.offset=saccade.offset+buffer)->
