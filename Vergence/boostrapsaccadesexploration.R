'
Plan: 
1) mark saccades with a good buffer
2) measure saccades
3) identify saccades meeting whatever criteria 
4) randomly choose n saccades 1999 times from that group (with replacement)
5) model and calculate confidence intervals
...
z %>%
  ...
  select(-rasters,-lep,-lev,-lepV,-levV,-rep,-rev,-repV,-revV,
         -sdf20,-time,-conj.velocity,-verg.angle,-verg.velocity,
         -counter,-cverg,-realsaccade,-issaccade)->
  zs

zs<- mutate(zs,verg.bins=cut(verg.amp,c(-50,-1,1,50)))

levels(zs$verg.bins)<- c('Diverging','Conjugate','Converging')
...
ggplot(zsn)+geom_bar(aes(verg.bins,n),stat='identity')+
  ggtitle('Types of saccades')+
  xlab('Saccade Type')+
  ylab('Number of saccades')
...
qplot(estimate,data=x,bins=30)+facet_wrap(~term,scales='free',ncol =1)

qplot(estimate,data=filter(x,term != '(Intercept)',term != 'verg.angle'),binwidth=0.0001)+
