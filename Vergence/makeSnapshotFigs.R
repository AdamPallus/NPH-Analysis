...
z<- filter(t,neuron=='Bee-15')
z<- filter(t,neuron=='Bee-33')
z<- filter(t,neuron=='Bee-09')

z$sdfsmooth=spikedensity(rasters=z$rasters,sd = 25)
# z<- mutate(z,lev=parabolicdiff(lep,n=20),
#            rev=parabolicdiff(rep,n=20),
...
#this snapshot from Bee-15 shows a clear divergence transient
#that is no encoded by the cell.
ggplot(filter(z,time>=104300,time<105600))+t
  geom_area(aes(time,sdf),alpha=1/10)+
  # geom_line(aes(time,slow.prediction),color='orange')+
  geom_line(aes(time,lev-rev),color='darkblue',alpha=1)+
  geom_line(aes(time,(lep-rep)*5-100),color='darkgreen')+
  geom_point(aes(time,showrasters+50),shape='|',size=2)+
  # geom_line(aes(time,enhance.velocity),size=2,color='darkblue')+
  theme_bw()
