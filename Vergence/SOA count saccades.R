countSaccades<- function(z){
  
  nConverge<- length(unique(z$sacnum[z$verg.bins=='Converging']))
  nDiverge<- length(unique(z$sacnum[z$verg.bins=='Diverging']))
  return(data.frame(nConverge=nConverge,nDiverge=nDiverge))
}
