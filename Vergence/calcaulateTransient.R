calculateTransient<- function(z) {
  ...
  z %>% 
    group_by(sacnum) %>%
    summarize(transient.onset=first(counter[transient.type=='divergence'])) %>%
  ...
  z%>%
    group_by(sacnum) %>%
    filter(!('divergence' %in% transient.type), 'convergence' %in% transient.type) %>%
    summarize(conv.onset=first(counter[!is.na(transient.type)])) %>%
