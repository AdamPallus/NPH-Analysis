...
path<- "C:/Users/setup/Desktop/NRTP Vergence/newcells/"
...
summaryforplot.old<- read.csv('allfiles-summaryforplot.csv')
summaryforplot.old<- summaryforplot.old[,-1]

#check to see if there are any new cells to avoid duplicating data
oldcells<- unique(summaryforplot.old$neuron)
loadedcells <- unique(summaryforplot$neuron)
newcells<- loadedcells[!(unique(summaryforplot$neuron) %in% unique(summaryforplot.old$neuron))]

if (length(newcells) > 0) {
  summaryforplot <- filter(summaryforplot,neuron %in% newcells)
  summaryforplot<- rbind(summaryforplot.old,summaryforplot)
  write.csv(summaryforplot,'allfiles-summaryforplot-new.csv',row.names=FALSE)
} else{
  summaryforplot<- summaryforplot.old
  print('No new cells. Ensure filenames are unique.')
}
...
# write.csv(p,'allfiles-p.csv')

p.old<- read.csv('allfiles-p.csv')
p.old<- p.old[,-1]

if (length(newcells) > 0) {
  p <- filter(p,neuron %in% newcells)
  p<- rbind(p.old,p)
  write.csv(summaryforplot,'allfiles-p-new.csv',row.names=FALSE)
} else{
  p<- p.old
}
