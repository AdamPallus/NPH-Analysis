...
path<- "C:/Users/setup/Desktop/NRTP Vergence/dontplot/"
...
```{r is.there.a.burst, fig.height=20,fig.width=13,warning=FALSE,message=FALSE}
...
g<- ggplot(aes(counter-buffer,sdf),data=filter(p,dur<maxdur))
...
```{r peak.verg.velocity,fig.height=10}
# qplot(peak.verg.velocity,maxfr,data=p,color=verg.amp)+facet_wrap(~neuron,ncol=1)+geom_point(size=3,alpha=1/20)
