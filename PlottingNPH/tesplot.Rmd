...
```{r peak.verg.velocity,fig.height=10,fig.width=5,echo=F}
...
p %>%
  ...
  # do(b=calc.relimp(lm(maxfr~verg.angle+peak.verg.velocity,data=.)),
     # bic=regsubsets(maxfr~verg.angle+peak.verg.velocity,.)) ->
    do(b=calc.relimp(lm(maxfr~verg.angle+min.verg.trans,data=.)),
     bic=regsubsets(maxfr~verg.angle+min.verg.trans,.)) ->
  bb
r <- data.frame()
for (i in 1:nrow(bb)){
  b<- bb$b[[i]]$lmg
...
ggplot(aes(verg.angle,min.verg.trans),data=r)+
  geom_point(size=4,aes(color=verg.angle>min.verg.trans))+
  geom_text(aes(label=neuron),check_overlap=TRUE,hjust="inward")+
  geom_abline(intercept=0,slope=1)+
  # coord_cartesian(xlim=c(-0.5,1),ylim=c(-0.5,1))+
  ggtitle('Relative Importance')
